<cfif DateDiff('d',NOW(),CreateDate(2008,08,31)) GTE 0>
...
<cfparam name="alternate_emails" default="">
...
<!--- Static variables --->
<cfset created_user_ID = 1000000069>
<cfset program_ID = 1000000069>
<cfset Points = 10>
<cfset HoldPoints = 0>
...
<cfif pgfn IS 'verify'>
	...
	<cfif LEN(branch_state2) GT 2><cfset ErrorMessage = ErrorMessage & 'Please enter the abbreviation of your state / province without spaces or punctuation<br />'></cfif>
	...
	<cfif trim(alternate_emails) NEQ "">
		<cfset badEmailFound = false>
		<cfset thisText = Replace(trim(alternate_emails),"#CHR(13)##CHR(10)#",",","ALL")>
		...
		<cfloop list="#thisText#" index="thisEmail">
			<cfif NOT REFind("^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6}$",trim(thisEmail))>
				<cfset badEmailFound = true>
				<cfbreak>
				<!--- <cfset ErrorMessage = ErrorMessage & '#trim(thisEmail)# is not a valid email address<br />'> --->
			</cfif>
		</cfloop>
		<cfif badEmailFound>
			<cfset ErrorMessage = ErrorMessage & 'Please check your Branch email entries; there is an invalid email address entered<br />'>
		<cfelseif ErrorMessage EQ "">
			<cfset alternate_emails = thisText>
		</cfif>
	</cfif>
	...
	<cflock name="program_userLock" timeout="60">
		...
		<cfif CheckProgramUser.RecordCount IS 0 AND CheckForRegister.recordcount IS 0>
			...
			<cfquery name="GetRegion" datasource="#application.DS#">
				...
				AND zipcode = <cfqueryparam value="#left(branch_zip2,3)#" cfsqltype="CF_SQL_VARCHAR">
			</cfquery>
			<cfset thisRegion = 0>
			...
			<cfquery name="CheckDistributor1" datasource="#application.DS#">
				...
				AND REPLACE(zip,' ','') = <cfqueryparam value="#replace(branch_zip2,' ','','ALL')#" cfsqltype="CF_SQL_VARCHAR">
			</cfquery>
			...
			<cfif status EQ 0>
				...
				<cfquery name="AddProgramUser" datasource="#application.DS#">
					INSERT INTO #application.database#.program_user (
						...
						ship_company, ship_address1, ship_city, ship_state, ship_zip, phone,
						email, is_active, idh, registration_type)
					VALUES (
				...
				<cfif isNumeric(getHoldPoints.totalpoints)>
					...
					<cfset Notes = Notes & " - #HoldPoints# from hold points">
			...
			<cfquery name="AddRegistration" datasource="#application.DS#">
				INSERT INTO #application.database#.henkel_register (
					created_user_ID, created_datetime, email, username, fname, lname, user_function,
					phone, company, is_international, address1, city, state, zip, country, region,
					program_ID, program_user_ID, status, registration_type, alternate_emails)
				VALUES (
					<cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#created_user_ID#" maxlength="10">,
					'#FLGen_DateTimeToMySQL()#',
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.email#" maxlength="128">,
					...
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.phone#" maxlength="35">,
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thisDistributor#" maxlength="64">,
					<cfqueryparam cfsqltype="cf_sql_tinyint" value="0" maxlength="30">,
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.branch_address2#" maxlength="64">,
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.branch_city2#" maxlength="30">,
					...
					<cfqueryparam cfsqltype="CF_SQL_LONGVARCHAR" value="#alternate_emails#" null="#len(trim(alternate_emails)) EQ 0#">
				)
			</cfquery>
			<cfif status EQ 0>
...
<cfif pgfn IS 'input'>							
	<cfoutput>
	<form action="#application.SecureWebPath#/henkel-register-cn.cfm" method="post" NAME="form_entry" onSubmit="return validateForm();">
		...
		<table border="0">
			...
			<tr><td align="right" class="welcome">Country </td><td>Canada</td></tr>
		...
		<table border="0">
			...
			<tr><td align="left" colspan="2">
				<table align="center" width="90%">
				<tr>
					<td valign="top"><input onClick="show_hide_alt_email();" name="registration_type" type="checkbox" value="1" <cfif registration_type EQ 1>checked</cfif>></td>
		...
		<script>
			function show_hide_alt_email() {
				var srcElement = document.getElementById('alt_email');
				if (document.form_entry.registration_type.checked == false ) {
		...
		<cfset thisDisplay = "none">
		<cfif registration_type EQ 1>
			<cfset thisDisplay = "block">
		</cfif>
		<div id="alt_email" style="display:#thisDisplay#;">
		<table border="0">
			<tr><td class="welcome" colspan="2">Please input a complete email address for each Branch participant, using a comma to separate each entry:</td></tr>
			<tr><td colspan="2"><textarea name="alternate_emails" rows="5" cols="60">#alternate_emails#</textarea></td></tr>
