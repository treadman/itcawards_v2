...
<cfif IsDefined("form.submitEmail")>
	<cfif not isDefined("form.changeHeader")>
		<cfset hasHeader = 0>
	</cfif>
	<cfset hasFile = true>
	<cftry>
	...
	<cfif NOT hasFile>
		<cfset alert_msg="Sorry, but the file was not found.  You will have to upload it again.">
...
<cfelseif url.pgfn EQ "email_setup">
	...
	<tr class="BGdark">
	<td colspan="2" class="TEXTheader"> First row is header  <input type="checkbox" name="changeHeader" value="1" <cfif hasHeader>checked</cfif> /> <i>Uncheck this if the first row is a data row.</i></td>
	</tr>
