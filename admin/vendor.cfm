...
<cfif IsDefined('form.Submit')>
	...
	<cfif form.pgfn EQ "edit">
		<cfquery name="UpdateQuery" datasource="#application.DS#">
			UPDATE #application.product_database#.vendor
			SET	vendor = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#vendor#" maxlength="60">,
				...
				email = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.email#" maxlength="512" null="#YesNoFormat(NOT Len(Trim(form.email)))#">,
		...
		<cflock name="vendorLock" timeout="10">
			<cftransaction>
				<cfquery name="InsertQuery" datasource="#application.DS#">
					INSERT INTO #application.product_database#.vendor
					...
					VALUES
						...
						<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.email#" maxlength="512" null="#YesNoFormat(NOT Len(Trim(form.email)))#">,
				...
				<cfquery name="getID" datasource="#application.DS#">
					SELECT Max(ID) As MaxID FROM #application.product_database#.vendor
				</cfquery>
...
<cfelseif delete NEQ '' AND FLGen_HasAdminAccess(1000000053)>
	<cfquery name="DeleteVendor" datasource="#application.DS#">
		DELETE FROM #application.product_database#.vendor
...
<cfif pgfn EQ "list">
	...
	<cfquery name="SelectList" datasource="#application.DS#">
		...
		LEFT JOIN #application.product_database#.vendor_lookup l ON v.ID = l.vendor_ID
		<cfif LEN(xT) GT 0>
	...
	<td align="right"><a href="<cfoutput>#CurrentPage#?show_inactive=#show_inactive#</cfoutput>" class="headertext">view all</a></td>
	</tr>
	...
	<td class="content" colspan="2" align="center">
		...
		<form name="SearchForm" action="#CurrentPage#" method="post">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="checkbox" name="show_inactive" value="1" <cfif show_inactive eq 1>checked</cfif> onclick="javascript:SearchForm.submit();"> Include inactive products on vendor product lists.
		</form>
		<br>		
		<cfif LEN(xL) IS 0><span class="ltrON">ALL</span><cfelse><a href="#CurrentPage#?show_inactive=#show_inactive#&xL=" class="ltr">ALL</a></cfif><span class="ltrPIPE">&nbsp;&nbsp;</span><cfloop index = "LoopCount" from = "0" to = "9"><cfoutput><cfif xL IS LoopCount><span class="ltrON">#LoopCount#</span><cfelse><a href="#CurrentPage#?show_inactive=#show_inactive#&xL=#LoopCount#" class="ltr">#LoopCount#</a></cfif></cfoutput><span class="ltrPIPE">&nbsp;&nbsp;</span></cfloop><cfloop index = "LoopCount" from = "1" to = "26"><cfoutput><cfif xL IS CHR(LoopCount + 64)><span class="ltrON">#CHR(LoopCount + 64)#</span><cfelse><a href="#CurrentPage#?show_inactive=#show_inactive#&xL=#CHR(LoopCount + 64)#" class="ltr">#CHR(LoopCount + 64)#</a></cfif></cfoutput><cfif LoopCount NEQ 26><span class="ltrPIPE">&nbsp;&nbsp;</span></cfif></cfloop>
		</cfoutput>
	</td>
	</tr>
	...
	<cfif SelectList.RecordCount GT 0>
		...
		<td>
			<cfif OnPage GT 1>
				<a href="<cfoutput>#CurrentPage#?show_inactive=#show_inactive#&OnPage=1&xL=#xL#&xT=#xT#</cfoutput>" class="pagingcontrols">&laquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?show_inactive=#show_inactive#&OnPage=#Max(DecrementValue(OnPage),1)#&xL=#xL#&xT=#xT#</cfoutput>" class="pagingcontrols">&lsaquo;</a>
			<cfelse>
		...
		<td align="right">
			<cfif OnPage LT TotalPages_SelectList>
				<a href="<cfoutput>#CurrentPage#?show_inactive=#show_inactive#&OnPage=#Min(IncrementValue(OnPage),TotalPages_SelectList)#&xL=#xL#&xT=#xT#</cfoutput>" class="pagingcontrols">&rsaquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?show_inactive=#show_inactive#&OnPage=#TotalPages_SelectList#&xL=#xL#&xT=#xT#</cfoutput>" class="pagingcontrols">&raquo;</a>
			<cfelse>
	...
	<cfoutput>	
		<tr class="contenthead">
		<td align="center"><a href="#CurrentPage#?show_inactive=#show_inactive#&pgfn=add&xL=#xL#&xT=#xT#&OnPage=#OnPage#">Add</a></td>
		<td>
	...
	<cfoutput query="SelectList" startrow="#StartRow_SelectList#" maxrows="#MaxRows_SelectList#">
		...
		<td><a href="#CurrentPage#?show_inactive=#show_inactive#&pgfn=edit&id=#ID#&xL=#xL#&xT=#xT#&OnPage=#OnPage#">Edit</a>&nbsp;&nbsp;&nbsp;<a href="#CurrentPage#?show_inactive=#show_inactive#&pgfn=copy&id=#ID#&xL=#xL#&xT=#xT#&OnPage=#OnPage#">Copy</a><cfif FLGen_HasAdminAccess(1000000053) and SelectList.numVendorProducts EQ 0>&nbsp;&nbsp;&nbsp;<a href="#CurrentPage#?show_inactive=#show_inactive#&delete=#ID#&xL=#xL#&xT=#xT#&OnPage=#OnPage#" onclick="return confirm('Are you sure you want to delete this vendor?  There is NO UNDO.')">Delete</a></cfif></td>
...
<cfelseif pgfn EQ "add" OR pgfn EQ "edit" OR pgfn EQ "copy">
	...
	<br /><br />
	<cfif pgfn eq 'edit'>
	<span class="pageinstructions">Open <a href="vendor_print.cfm?show_inactive=#show_inactive#&id=#id#" target="_blank">printable</a> page.</span>
	<br /><br />
	</cfif>
	<span class="pageinstructions">Return to <a href="#CurrentPage#?show_inactive=#show_inactive#&xL=#xL#&xT=#xT#&OnPage=#OnPage#">Vendor List</a> without making changes.</span>
	<br /><br />
	</cfoutput>
	...
	<tr class="contenthead">
	<td colspan="2" class="headertext"><cfif pgfn EQ "add" OR pgfn EQ "copy">Add<cfelse>Edit</cfif> a Vendor</td>
	</tr>

	<tr class="content">
	<td align="right" valign="top">Vendor Name: </td>
	<td valign="top"><input type="text" name="vendor" value="#vendor#" maxlength="60" size="40"></td>
	</tr>
	
	<tr class="content">
	<td align="right" valign="top">Address Line 1: </td>
	<td valign="top"><input type="text" name="address1" value="#address1#" maxlength="38" size="40"></td>
	</tr>
	
	<tr class="content">
	<td align="right" valign="top">Address Line 2: </td>
	<td valign="top"><input type="text" name="address2" value="#address2#" maxlength="38" size="40"></td>
	</tr>
		
	<tr class="content">
	<td align="right" valign="top">City: </td>
	<td valign="top"><input type="text" name="city" value="#city#" maxlength="38" size="40"></td>
	</tr>
	<tr class="content">
	...
	<td valign="top"><input type="text" name="fax" value="#fax#" maxlength="14" size="40"></td>
	</tr>
		
	<tr class="content">
	<td align="right" valign="top">Email: </td>
	<td valign="top"><input type="text" name="email" value="#email#" maxlength="512" size="120"></td>
	</tr>
		
	<tr class="content">
	<td align="right" valign="top">Attention: </td>
	<td valign="top"><input type="text" name="attention" value="#attention#" maxlength="32" size="40"></td>
	</tr>
	...
	<td valign="top"><textarea name="notes" cols="60" rows="10">#notes#</textarea></td>
	</tr>
	...
	<input type="hidden" name="vendor_required" value="You must enter a vendor name.">
	...
	<cfif pgfn EQ "edit">
		...
		<cfquery name="FindVendorsProds" datasource="#application.DS#">
			SELECT m.ID as meta_id, m.meta_sku AS meta_sku, m.meta_name AS meta_name, p.productvalue AS productvalue, prod.sku AS sku,
					prod.ID AS product_ID, IF(prod.is_dropshipped=1,'DROP','') AS is_dropshipped, prod.is_active, vl.vendor_sku
			FROM #application.product_database#.product_meta m
				JOIN #application.product_database#.productvalue_master p ON m.productvalue_master_ID = p.ID 
				JOIN #application.product_database#.product prod ON prod.product_meta_ID = m.ID 
				JOIN #application.product_database#.vendor_lookup vl ON vl.product_ID = prod.ID 
			WHERE vl.vendor_ID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#ID#" maxlength="10">
			<cfif show_inactive eq 0>
		...
		<cfelse>
			...
			<tr class="contenthead">
			<td colspan="100%" class="headertext">
				Individual Products assigned to this Vendor
				<cfif show_inactive eq 1>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<em>Showing inactive products</em>
				</cfif>
			</td>
			</tr>
			...
			<td><span class="headertext">Vendor&nbsp;SKU&nbsp;&nbsp;&nbsp;</span></td>
			<td><span class="headertext">CAT</td>
			<td><span class="headertext">Product Name</span></td>
			<cfif show_inactive eq 1>
			<td align="center"><span class="headertext">Active?</span></td>
			</cfif>
			</tr>
			...
			<td><a href="product.cfm?pgfn=edit&meta_id=#meta_id#">#HTMLEditFormat(sku)#</a></td>
			<td>#HTMLEditFormat(vendor_sku)#</td>
			<td>#productvalue#</td>
			<td>#meta_name# #FindProductOptions(product_ID)##FPO_theseoptions#</td>
			<cfif show_inactive eq 1>
				<td align="center"><cfif FindVendorsProds.is_active>Yes<cfelse>No</cfif></td>
			</cfif>
			</tr>
				</cfoutput>
			</cfloop>
