...
<cfif pgfn EQ "list">
	...
	<!--- <cfquery name="SelectList" datasource="#application.DS#">
		...
		WHERE 1=1 
		<cfif LEN(xT) GT 0>
			AND email_group_name LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#PreserveSingleQuotes(xT)#%">
		</cfif>
		<cfif cri_C NEQ "">
			AND program_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#cri_C#">
		</cfif>
		ORDER BY program_ID, emailgroup_name ASC
	</cfquery> --->
	<cfquery name="SelectList" datasource="#application.DS#">
		...
		<cfif cri_C NEQ "">
			AND g.program_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#cri_C#">
		</cfif>
		GROUP BY g.ID
	...
	<span class="pagetitle">Email Alert Group List</span>
	<br /><br />
	...
	<td class="content" colspan="2" align="center">
		...
		<form action="#CurrentPage#" method="post">
			award program:
			<select name="cri_C">
				<option value="">view groups for all Awards Program</option>
			<cfloop query="SelectPrograms">
			<option value="#SelectPrograms.ID#"<cfif cri_C EQ #SelectPrograms.ID#> selected</cfif>>#SelectPrograms.company_name# [#SelectPrograms.program_name#]</option>
			</cfloop>
			</select>
			<br><br>
	...
	<td>
		<cfif OnPage GT 1>
			<a href="<cfoutput>#CurrentPage#?OnPage=1&xT=#xT#&cri_C=#cri_C#</cfoutput>" class="pagingcontrols">&laquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?OnPage=#Max(DecrementValue(OnPage),1)#&xT=#xT#&cri_C=#cri_C#</cfoutput>" class="pagingcontrols">&lsaquo;</a>
		<cfelse>
	...
	<select name="pageselect" onChange="openURL()"> 
		<cfloop from="1" to="#TotalPages_SelectList#" index="this_i">
			<option value="#CurrentPage#?OnPage=#this_i#&xT=#xT#&cri_C=#cri_C#"<cfif OnPage EQ this_i> selected</cfif>>#this_i#</option> 
		</cfloop>
	...
	<td align="right">
		<cfif OnPage LT TotalPages_SelectList>
			<a href="<cfoutput>#CurrentPage#?OnPage=#Min(IncrementValue(OnPage),TotalPages_SelectList)#&xT=#xT#&cri_C=#cri_C#</cfoutput>" class="pagingcontrols">&rsaquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?OnPage=#TotalPages_SelectList#&xT=#xT#&cri_C=#cri_C#</cfoutput>" class="pagingcontrols">&raquo;</a>
		<cfelse>
	...
	<cfoutput>	
	<tr class="contenthead">
		<td align="center"><a href="#CurrentPage#?pgfn=add&xT=#xT#&cri_C=#cri_C#&OnPage=#OnPage#">Add</a></td>
	...
	<cfoutput query="SelectList" startrow="#StartRow_SelectList#" maxrows="#MaxRows_SelectList#">
		<!--- <cfquery name="GroupCount" datasource="#application.DS#">
		...
		<td nowrap="nowrap"><a href="#CurrentPage#?pgfn=edit&id=#ID#&xT=#xT#&cri_C=#cri_C#&OnPage=#OnPage#">Edit</a>&nbsp;&nbsp;&nbsp;<a href="#CurrentPage#?delete=#ID#&xT=#xT#&cri_C=#cri_C#&OnPage=#OnPage#" onclick="return confirm('Are you sure you want to delete this email group?  There is NO UNDO.')">Delete</a></td>
...
<cfelseif pgfn EQ "add" OR pgfn EQ "edit">
	...
	<br /><br />
	<span class="pageinstructions">Return to <a href="#CurrentPage#?xT=#xT#&cri_C=#cri_C#&OnPage=#OnPage#">Email Alert Group List</a> without making changes.</span>
	<br /><br />
	</cfoutput>
	...
	<td valign="top"><input type="text" name="emailgroup_name" value="#emailgroup_name#" maxlength="40" size="40"><input type="hidden" name="emailgroup_name_required" value="You must enter a name for the email alert group."></td>
	</tr>
	
	<!--- Program Choose or Display --->
	<cfif pgfn EQ 'add'>
	...
	<tr class="content">
	<td align="right" valign="top">Award Program: </td>
	<td valign="top">
		<select name="program_ID">
				<option value="">Choose an Award Program</option>
			<cfloop query="SelectPrograms">
			<option value="#SelectPrograms.ID#"<cfif cri_C EQ #SelectPrograms.ID#> selected</cfif>>#SelectPrograms.company_name# [#SelectPrograms.program_name#]</option>
			</cfloop>
		</select>
		<input type="hidden" name="program_ID_required" value="You must select an Award Program.">
	</td>
	</tr>
	...
	<tr class="content">
	<td align="right" valign="top">Program: </td>
	<td valign="top">#FLITC_GetProgramName(program_ID)#</td>
	</tr>
	...
	<td colspan="2" align="center">
		...
		<input type="reset" value="Reset" >&nbsp;&nbsp;&nbsp;<input type="submit" name="submit" value="#save_button#" >
	</td>
	</tr>
