...
<cfset program_name = "">
<cfif vendor_ID NEQ "">
	<cfset program_name = FLITC_GetVendorName(vendor_ID)>
</cfif>

<cfoutput>
<span class="pagetitle">PO Quantity Report<cfif program_name NEQ ""> for #program_name#</cfif></span>
<br /><br />
...
<cfif IsDefined('form.submit')>
	...
	<cfquery name="getProducts" datasource="#application.DS#">
		...
		JOIN #application.database#.purchase_order po ON inv.po_ID = po.ID
	...
	<table cellpadding="5" cellspacing="1" border="0" width="100%">
		<!--- header row --->
		<tr class="content2">
			<td colspan="5"><span class="headertext">Vendor: <span class="selecteditem"><cfif vendor_ID NEQ ""><cfoutput>#program_name#</cfoutput><cfelse>All Vendors</cfif></span></span></td>
		</tr>
		...
		<cfelse>
			<cfoutput query="getProducts">
				<!--- qty ordered --->
				<!--- <cfquery name="ThisTotalQTY" datasource="#application.DS#">
					SELECT 
						<cfif snap_is_dropshipped EQ 0>
							SUM(po_quantity) 
						<cfelse>
							SUM(quantity) 
						</cfif>
						AS thistotalquantity
					FROM #application.database#.inventory inv
					JOIN #application.database#.purchase_order po ON inv.po_ID = po.ID
					WHERE inv.is_valid = 1
						AND inv.product_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#product_ID#" maxlength="10">
						AND inv.po_ID <> 0
						<cfif formatFromDate NEQ "">
							AND inv.created_datetime >= '#formatFromDate#' 
						</cfif>	
				...
				<!--- qty delivered --->
				<!--- <cfquery name="ThisTotalREC" datasource="#application.DS#">
					SELECT SUM(quantity) AS thistotalquantity
					FROM #application.database#.inventory inv
					JOIN #application.database#.purchase_order po ON inv.po_ID = po.ID
					WHERE inv.is_valid = 1
						AND inv.product_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#product_ID#" maxlength="10">
