...
<cfif IsDefined('form.Submit')>
	...
	<cfif found_one>
		...
		<cflocation addtoken="no" url="po_detail.cfm?pgfn=detail&po_ID=#new_po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#&new=yes">
	<cfelse>
		<cfset alert_msg = 'You must enter a value for the products you want to add to the new purchase order.'>
	</cfif>
...
<br />
<span class="pageinstructions">You can change quantities and after the Purchase Order is created.</span>
<br /><br />
...
<cfquery name="FindVenProd" datasource="#application.DS#">
	...
	FROM #application.product_database#.vendor_lookup vl
		...
		JOIN #application.product_database#.product_meta m ON m.ID = prod.product_meta_ID 
		JOIN #application.product_database#.productvalue_master pvm ON pvm.ID = m.productvalue_master_ID
	...
	<cfloop query="FindVenProd">
		...
		<cfif is_dropshipped EQ 0>
			...
			<cfif AnyUnShipped.RecordCount GT 0>
				<cfoutput>#PhysicalInvCalc(product_ID)#</cfoutput>
				...
				<cfif PIC_total_virtual LT 0>
					...
					<td>
						<input type="text" name="prod_#product_ID#" size="6" maxlength="6">
