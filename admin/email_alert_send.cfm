...
<cffunction name="FLITCAwards_FindRecipients" output="true">
	<cfargument name="FLITCAwards_ProgramID" required="yes">
	...
	<cfquery name="SelectProgramInfo" datasource="#application.DS#">
		...
		WHERE ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#FLITCAwards_ProgramID#" maxlength="10">
	</cfquery>
	<cfset points_multiplier = SelectProgramInfo.points_multiplier>
	<cfif FLITCAwards_ChosenRecipients EQ 11>
		<cfquery name="FLITCAwards_SelectRecipients" datasource="#application.DS#">
			SELECT email, email as ListText, '' as fname, '' as lname, '' AS expiration_date, SUM(points) AS remaining_points, '' as supervisor_email, '' as level_of_award, '' as username, '' as subprogram_points 
			FROM #application.database#.henkel_hold_user
			WHERE email REGEXP '^[^@]+@[^@]+\.[^@]{2,}$'
			<!--- WHERE created_datetime > '2012-01-01' AND created_datetime < '2017-01-01'--->
			GROUP BY email
		</cfquery>
	<cfelse>
	<cfquery name="FLITCAwards_SelectRecipients" datasource="#application.DS#">
		SELECT ID AS this_user_ID, fname, lname, email, Date_Format(expiration_date,'%c/%d/%Y') AS expiration_date, Format((((SELECT IFNULL(SUM(points),0) FROM #application.database#.awards_points WHERE user_ID = program_user.ID AND is_defered = 0) - (SELECT IFNULL(SUM(points_used),0) FROM #application.database#.order_info WHERE created_user_ID = program_user.ID AND is_valid = 1)) * #SelectProgramInfo.points_multiplier#),0) AS remaining_points, supervisor_email, level_of_award, username, 
		<cfif FLITCAwards_ChosenRecipients EQ 10>
		Format(((SELECT IFNULL(SUM(subpoints),0) FROM #application.database#.subprogram_points 
			WHERE user_ID = program_user.ID 
		...
		<cfif FLITCAwards_OnlyEmails EQ true>
			email AS ListText
		<cfelse>
			CONCAT(fname,' ',lname,', <b>',email,IF(TRIM(supervisor_email) <> '',' has a supervisor',''),'</b> ') AS ListText
			<!--- CONCAT(IFNULL(fname,'(no first name)'),' ',IFNULL(lname,'(no last name)'),', <b>',email,IF(TRIM(supervisor_email) <> '',' has a supervisor',''),'</b> ',
				Format((((SELECT IFNULL(SUM(points),0) FROM #application.database#.awards_points WHERE user_ID = program_user.ID AND is_defered = 0) - (SELECT IFNULL(SUM(points_used),0) FROM #application.database#.order_info WHERE created_user_ID = program_user.ID AND is_valid = 1))* #SelectProgramInfo.points_multiplier#),0),' points'
			<cfif FLITCAwards_ChosenRecipients EQ 10>
					,' and ',Format(((SELECT IFNULL(SUM(subpoints),0) FROM #application.database#.subprogram_points WHERE user_ID = program_user.ID AND subprogram_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#sub_ID#">) * #SelectProgramInfo.points_multiplier#),0),' subpoints'
		...
		WHERE program_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#FLITCAwards_ProgramID#">
			AND email <> '' AND email like '%@%'
			...
			<cfif FLITCAwards_ChosenRecipients EQ 3 OR FLITCAwards_ChosenRecipients EQ 5 OR FLITCAwards_ChosenRecipients EQ 6>
				...
				(SELECT IFNULL(SUM(points),0) FROM #application.database#.awards_points WHERE user_ID = program_user.ID AND is_defered = 0) 
				- 
				(SELECT IFNULL(SUM(points_used),0) FROM #application.database#.order_info WHERE created_user_ID = program_user.ID AND is_valid = 1)
				> 0
			</cfif>
			...
			<cfif FLITCAwards_ChosenRecipients EQ 10 AND sub_ID NEQ "">
				AND 
				(SELECT IFNULL(SUM(subpoints),0) FROM #application.database#.subprogram_points WHERE user_ID = program_user.ID AND subprogram_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#sub_ID#">)
				> 0
			</cfif>
	...
	<cfif FLITCAwards_return EQ "true">
		...
		<cfelseif FLITCAwards_HasRadio EQ true>
			<cfloop query="FLITCAwards_SelectRecipients">
				<input type="radio" name="spoof" value="<cfif FLITCAwards_ChosenRecipients EQ 11>#email#<cfelse>#this_user_ID#</cfif>"<cfif CurrentRow EQ 1> checked</cfif>> #ListText#<br>
			</cfloop>
...
function submitLinkForm(current_panel,next_panel,FromField,ToField) {
	...
	if (current_panel == "1_pickprogram") {
		...
		if (FromValue == "") {
			alert("You must select a program."); return false
		} else {
	...
	if (FromField == 's_fillin') {
		if (trim(document.getElementById('s_fillin').value) == "") {
			alert("You must enter the fill-in-the-blank text.\n\nIf you do not want fill-in-the-blank text, you need to\nedit this template (click Templates under Email Alerts)\nbefore you can send this broadcast.");
			return false;
		} else {
			FromField = document.getElementById(FromField)
	...
	if (FromField == 's_recipients') {
		...
		if ((FromValue == "4" || FromValue == "5") && !validateDate()) {
			alert('You must enter a vaild date.'); return false
		} else {
			ToField = document.getElementById(ToField)
			ToField.value = FromValue
		}
		if (FromValue == "6" && document.getElementById('s_award_level').value == "") {
			alert('You must enter a level of award.'); return false
		...
		if (FromValue == "7" && document.getElementById('s_date_of_award').value == "") {
			alert('You must enter a date of award.'); return false
		...
		if (FromValue == "9" && trim(document.getElementById('s_point_total').value) == "") {
			alert('You must enter a point total.'); return false
		...
		if (FromValue == "10" && document.getElementById('s_subprogram').value == "") {
			alert('You must choose a subprogram.'); return false
	...
	if (current_panel == "5_preview") {
		if (document.getElementById('s_email_subject').value == "" || document.getElementById('email_test').value == "" || document.getElementById('s_from_email').value == "") {
			alert('You must complete all three fields: email subject, to email, and from email.'); return false
		} else {
...
function openPreview() {
	...
	programIdValue = programId.value
	
	// open new window with preview	
	TheWindow = window.open('email_alert_preview.cfm?ID='+previewIdValue+'&prog='+programIdValue,'windowname');
...
<span class="pagetitle">Send An Email Alert</span>
<br /><br />

<!--- START / PAGE SET UP --->

<!--- if a Program Admin, skip to panel 2 --->
<cfparam name="panel_number" default="">
<cfif panel_number EQ "" AND FLGen_HasAdminAccess(1000000073)>
	<cfset panel_number = "1_pickprogram">
<cfelseif panel_number EQ "" AND FLGen_HasAdminAccess(1000000075)>
	<cfset panel_number = "2_picktemplate">
</cfif>

<!--- set program ID from cookie for Program Admin users --->
<cfif FLGen_HasAdminAccess(1000000075)>
	<cfset program_ID = ListGetAt(cookie.itc_program,1,"-")>
</cfif>

<!--- whether to show "go back to the start" link --->
<cfif (panel_number NEQ "1_pickprogram" AND FLGen_HasAdminAccess(1000000073)) OR (panel_number NEQ "2_picktemplate" AND FLGen_HasAdminAccess(1000000075))>
...
<cfparam name="from_email" default="">
...
<form action="#CurrentPage#" method="post" name="send_email_form" id="send_email_form" onkeypress="return event.keyCode!=13">
	...
	<input type="hidden" name="from_email" id="from_email" value="#application.AwardsFromEmail#" />
	...
	<cfif program_ID NEQ "">
		<cfset choosen_program = #FLITC_GetProgramName(program_ID)#>
	</cfif>
	...
	<cfif recipients NEQ "">
		<cfswitch expression="#recipients#">	
			...
			<cfcase value="11">
				<cfset choosen_recipients = "All Holding Tank users">
			</cfcase>
		</cfswitch>
	</cfif>
	...
	<!--- 1. pick a program --->
	<cfif panel_number EQ "1_pickprogram">
	
		<table cellpadding="5" cellspacing="1" border="0" width="100%">
		<tr class="contenthead">
		<td colspan="2" class="headertext">Select Program</td>
		</tr>
		<tr class="content">
		<td>#SelectProgram(SelectProgram_selectname="s_program_ID",SelectProgram_selectID="s_program_ID",SelectProgram_onlyactive="true")#</td>
		</tr>
		<tr class="content">
		<td><input type="submit" value="Select This Program" name="submit_button" onclick="submitLinkForm('1_pickprogram','2_picktemplate','s_program_ID','program_ID');return false;"></td>
		</tr>
		</table>

	<!--- 2. pick an email template --->
	<cfelseif panel_number EQ "2_picktemplate">
		<cfquery name="SelectEmailTemplates" datasource="#application.DS#">
			...
			WHERE ea.is_available = 1
				AND xref.program_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#program_ID#">
	...
	<cfelseif panel_number EQ "3_fillintheblank">
		...
		<tr class="content">	
		<td colspan="2"><span class="sub">Template:</span> #choosen_template#&nbsp;&nbsp;&nbsp;&nbsp;<a href="email_alert_preview.cfm?ID=#template_ID#&prog=#program_ID#" target="_blank">preview</a></td>
		</tr>
		<tr class="content">
		<td>
		...
		<table cellpadding="5" cellspacing="1" border="0" width="100%">
			...
			<tr class="content">
				<td>
					...
					<input type="radio" name="s_recipients" value="11" onClick="hideThis('point_line');hideThis('date_line');hideThis('group_line');hideThis('subprogram_line');"> All Holding Tank users<br>
					...
					<!--- <input type="radio" name="s_recipients" value="6" onClick="showThis('award_line');hideThis('date_line');hideThis('award_date_line');hideThis('group_line');"> Program users who are not expired, have a certain level of award, and have points remaining<br>
					<span id="award_line" style="display:none"><br>
					Level of Award: <input type="text" name="s_award_level" id="s_award_level" size="15" maxlength="10"></span> --->
					<!--- <input type="radio" name="s_recipients" value="7" onClick="showThis('award_date_line');hideThis('date_line');hideThis('award_line');hideThis('group_line');"> Program users who are not expired, have a certain date of award, and have points remaining<br>
					<span id="award_date_line" style="display:none"><br>
					Date of Award: <input type="text" name="s_date_of_award" id="s_date_of_award" size="15" maxlength="10"></span> --->
	...
	<cfelseif panel_number EQ "5_preview">
		...
		<br>#FLITCAwards_FindRecipients(program_ID,recipients,date_expire,award_level,date_of_award,false,true,true)#<br><cfif points_multiplier NEQ 1><br><span class="sub">(users' points shown times credit multiplier)</span></cfif></td>
		</tr>
		<tr class="content">
		<td>Email Subject: <input type="text" name="s_email_subject" id="s_email_subject" size="40" maxlength="60"></td>
		</tr>
		<tr class="content">
		<td>Send Email From: <input type="text" name="s_from_email" id="s_from_email" size="40" maxlength="128" value="#application.AwardsFromEmail#" readonly></td>
		</tr>
		<tr class="content">
	...
	<cfelseif panel_number EQ "6_sendtestemail">
		...
		<!--- find spoof info --->
		<cfif recipients EQ 11>
			<cfquery name="SpoofInfo" datasource="#application.DS#">
				SELECT email, '' as fname, '' as lname, '' as expiration_date, SUM(points) AS remaining_points, '' as level_of_award, '' as username 
				FROM #application.database#.henkel_hold_user
				WHERE email = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.spoof#">
				GROUP BY email
			</cfquery>
		<cfelse>
			<cfquery name="SpoofInfo" datasource="#application.DS#">
		...
		<cfmail failto="#application.AwardsProgramAdminEmail#" to="#form.email_test#" from="#from_email#" subject="TEST - #email_subject#" type="html"><font face="Arial, Helvetica, sans-serif">
<br><span class="alert" style="letter-spacing:3PX;padding-left:30px">EMAIL ALERT PREVIEW</span><br><br>
<hr size="1" width="100%">	
#email_text#
			<!--- Set Email Attachment --->
			<cfif FileExists(application.AbsPath & "award_certificate/" & SpoofInfo.username & "_certificate_" & program_ID & ".pdf")>
				<cfmailparam file = "/award_certificate/#SpoofInfo.username#_certificate_#program_ID#.pdf" type ="application/pdf">
			</cfif>
		</cfmail>
		<!--- END Send Test Email --->
		...
		<td style="padding-left:30px">
			#FLITCAwards_FindRecipients(program_ID,recipients,date_expire,award_level,date_of_award,false,true,false,true)#<br><cfif points_multiplier NEQ 1><span class="sub">(users' points shown times credit multiplier)</span><br></cfif>
		</td>
		</tr>
		<tr class="content">
	...
	<cfelseif panel_number EQ "7_sendemail">
		...
		<cfset email_text = Replace(email_text,"DATE-TODAY",DateFormat(Now(),'m/d/yyyy'),"all")>
		<cfset FLITCAwards_FindRecipients(program_ID,recipients,date_expire,award_level,date_of_award,false,false)>
		...
		<cfloop query="FLITCAwards_SelectRecipients">
			...
			<cftry>
				<!--- Send Email Alert --->
				<cfmail failto="#application.AwardsProgramAdminEmail#" to="#FLITCAwards_SelectRecipients.email#" from="#from_email#" subject="#email_subject#" type="html">
#user_email_text#

					<!--- Set Email Attachment --->
					<cfif attach_cert EQ "1">
						<cfif FileExists(application.AbsPath & "award_certificate/" & FLITCAwards_SelectRecipients.username & "_certificate_" & program_ID & ".pdf")>
							<cfmailparam file = "/award_certificate/#FLITCAwards_SelectRecipients.username#_certificate_#program_ID#.pdf" type ="application/pdf">
						</cfif>
					</cfif>
				</cfmail>
			...
			<cfif send_sup_email EQ "1">
				<cfif FLITCAwards_SelectRecipients.supervisor_email NEQ ""> 
					<cftry>
						<cfmail failto="#application.AwardsProgramAdminEmail#" to="#FLITCAwards_SelectRecipients.supervisor_email#" from="#from_email#" subject="Supervisor Copy - #email_subject#" type="html">
...
#user_email_text#
			...
			<cfif email_sent>
				...
				<cfcatch>
					<span class="alert">#snap_remaining_points#</span> is not a valid point value for user #FLITCAwards_SelectRecipients.email# in email number #alert_ID#.<br> 
				</cfcatch>
				</cftry>
			</cfif>
		</cfloop>
