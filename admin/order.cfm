...
<cfif IsDefined('form.Submit')>
	...
	<cfelseif IsDefined('form.edit') AND form.edit EQ 'editorderitem'>
		...
		<cfif quantity GT original_quantity AND quantity NEQ 0>
			...
			<cfoutput>#ProgramUserInfo(GetUserID.created_user_ID)#</cfoutput>
			...
			<cfelse>
				...
				<cfset alert_msg = "NOT SAVED. This user has #user_totalpoints# credits available.#FLGen_SubStamp()#">
				<cfset pgfn = "edititem">
			</cfif>
	...
	<cfelseif IsDefined('form.edit') AND #form.edit# EQ 'addorderitem'>
		...
		<cfif user_totalpoints GTE fm_productvalue>
			...
			<cfoutput>#FindProductOptions(fm_individual_ID)#</cfoutput>
		...
		<cfelse>
			<cfset pgfn = "additem">
			<cfset alert_msg = "The user doesn't have enough points for that product.">
		</cfif>	
	</cfif>
...
<cfif pgfn EQ "list">
	<cfif isNumeric(program_ID)>
		<cfquery name="summary" datasource="#application.DS#">
			SELECT YEAR(o.created_datetime) AS order_year, COUNT(o.created_user_ID) AS num_users,
			SUM(o.snap_order_total) AS order_total, SUM(o.points_used) AS points_total, SUM(o.cc_charge) AS credit_card
			FROM #application.database#.order_info o
			LEFT JOIN #application.database#.program p ON o.program_ID = p.ID
			WHERE o.is_valid = 1 AND o.program_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#program_ID#" maxlength="10">
			GROUP BY YEAR(o.created_datetime)
			ORDER BY YEAR(o.created_datetime), o.created_user_ID
		</cfquery>
	</cfif>
	<!--- massage dates --->
	...
	<cfif (only_unfulfilled OR xFD NEQ "" OR xTD NEQ "" OR xT NEQ "" OR this_from_date NEQ "" OR this_to_date NEQ "" OR program_ID NEQ "") OR CGI.REQUEST_METHOD EQ "Post">
		<!--- run query --->
		<cfquery name="SelectList" datasource="#application.DS#">
			...
			ORDER BY pg.company_name, pg.program_name, oi.order_number
		</cfquery>
		...
		<form action="#CurrentPage#" method="post">
			<table cellpadding="5" cellspacing="0" border="0" width="100%">
				...
				<cfif NOT request.newNav>
					<span class="sub">award program</span><br>#SelectProgram(program_ID,"All Award Programs","program_ID","program_ID",active_only)#
				</cfif>
				<br><br>
	...
	<cfif isDefined("summary")>
		<table cellpadding="0" cellspacing="0" border="0" width="60%">
			<tr class="contenthead">
				<td align="right" class="headertext">Year</td>
				<td align="right" class="headertext">Users</td>
				<td align="right" class="headertext">Points Used</td>
				<td align="right" class="headertext">CC Used</td>
				<td align="right" class="headertext">Order Total</td></tr>
			<cfoutput query="summary">
				<tr class="#Iif(((CurrentRow MOD 2) is 1),de('content2'),de('content'))#">
				<td align="right">#order_year#</td><td align="right">#num_users#</td><td align="right">#points_total#</td><td align="right">#credit_card#</td><td align="right">#order_total#</td></tr>
			</cfoutput>
			<tr height="3px;" class="contenthead"><td colspan="100%"></td></tr>
		</table>
	</cfif>
	<cfif IsDefined('SelectList.RecordCount') AND SelectList.RecordCount GT 0>
		<form name="pageform">
			...
			<td>
				<cfif OnPage GT 1>
					<a href="<cfoutput>#CurrentPage#?OnPage=1&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#</cfoutput>" class="pagingcontrols">&laquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?OnPage=#Max(DecrementValue(OnPage),1)#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#</cfoutput>" class="pagingcontrols">&lsaquo;</a>
				<cfelse>
			...
			<td align="center" class="sub">[ page 	
				...
				<select name="pageselect" onChange="openURL()"> 
					<cfloop from="1" to="#TotalPages_SelectList#" index="this_i">
						<option value="#CurrentPage#?OnPage=#this_i#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#"<cfif OnPage EQ this_i> selected</cfif>>#this_i#</option> 
					</cfloop>
				</select>
			...
			<td align="right">
				<cfif OnPage LT TotalPages_SelectList>
					<a href="<cfoutput>#CurrentPage#?OnPage=#Min(IncrementValue(OnPage),TotalPages_SelectList)#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#</cfoutput>" class="pagingcontrols">&rsaquo;</a>&nbsp;&nbsp;&nbsp;<a href="<cfoutput>#CurrentPage#?OnPage=#TotalPages_SelectList#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#</cfoutput>" class="pagingcontrols">&raquo;</a>
				<cfelse>
	...
	<cfif (only_unfulfilled OR xFD NEQ "" OR xTD NEQ "" OR xT NEQ "" OR this_from_date NEQ "" OR this_to_date NEQ "" OR program_ID NEQ "") OR CGI.REQUEST_METHOD EQ "Post">
		...
		<cfoutput query="SelectList" startrow="#StartRow_SelectList#" maxrows="#MaxRows_SelectList#">
			...
			<td align="center"><a href="#CurrentPage#?pgfn=detail&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Detail</a></td>
...
<cfelseif pgfn EQ "detail">
	...
	<cfquery name="FindOrderInfo" datasource="#application.DS#">
		SELECT ID AS order_ID, program_ID AS this_program_ID, order_number, snap_order_total, points_used,
			credit_multiplier, points_multiplier, created_user_ID,
			cc_charge, snap_fname, snap_lname, snap_ship_company, snap_ship_fname, snap_ship_lname,
	...
	<cfset cc_charge = HTMLEditFormat(FindOrderInfo.cc_charge)>
	...
	<cfquery name="FindProgramInfo" datasource="#application.DS#">
		SELECT company_name, program_name, IF(is_one_item=1,"true","false") as is_one_item, credit_desc
	...
	<span class="pageinstructions">Return to <a href="#CurrentPage#?xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Order List</a> without making changes.</span>
	<br /><br />
	...
	<tr>
	<td colspan="2" class="content2"><b>Award Program:</b> <span class="selecteditem">#company_name# [#program_name#]</span><cfif is_one_item> <span class="sub">(this is a one-item program)</span></cfif></td>
	</tr>
	...
	<tr>
	<td colspan="2" class="content2"><a href="#CurrentPage#?pgfn=edit&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Edit Order Information</a></td>
	</tr>
	...
	<cfquery name="UserForwarding" datasource="#application.DS#">
		...
		WHERE u.ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#FindOrderInfo.created_user_ID#">
	</cfquery>
	...
	<td>
	<cfif hasForwarding>
		Ship to forwarder address:<br>
		#UserForwarding.company#<br>
		<cfif UserForwarding.address1 NEQ "">#UserForwarding.address1#<br></cfif>
		<cfif UserForwarding.address2 NEQ "">#UserForwarding.address2#<br></cfif>
		<cfif UserForwarding.city NEQ "">#UserForwarding.city#,</cfif> <cfif UserForwarding.state NEQ "">#UserForwarding.state#</cfif> <cfif UserForwarding.zip NEQ "">#UserForwarding.zip#</cfif><br>
		<br>
		Forward to:<br>
	</cfif>
	...
	<cfif FindOrderItems.RecordCount EQ 0>
		There are no products in this order. <a href="#CurrentPage#?pgfn=deleteorder&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#" onclick="return confirm('Are you sure you want to delete this order?  There is NO UNDO.')">Delete this order.</a>
		<br /><br />
		<a href="#CurrentPage#?pgfn=additem&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Add a product to this order.</a>
	<cfelse>
		<table cellpadding="3" cellspacing="1" border="0" width="100%">
		<tr class="contenthead">
		<td><a href="#CurrentPage#?pgfn=additem&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Add</a></td>
		<td><b>SKU</b></td>
		...
		<cfset carttotal = 0>
	 	<cfloop query="FindOrderItems">
			<tr class="content">
			<td><a href="#CurrentPage#?pgfn=edititem&inventory_ID=#inventory_ID#&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Edit</a></td>
			<td>#snap_sku#</td>
			...
			<td>
				<cfif snap_is_dropshipped EQ 1>
					<cfif drop_date EQ "">
						not dropshipped yet
					<cfelse>
						DROP PO #RemoveChars(po_ID,1,5)#<br>
		...
		<cfif NOT is_one_item>
			...
			<tr>
			<td align="right" colspan="6"> <b>Credit Card <cfif x_tran_ID NEQ "">Transaction: #x_tran_ID#<cfelse>Charge</cfif>:</b> </td>
			<td align="right" class="content"><cfif cc_charge NEQ "" AND cc_charge NEQ 0><b>#NumberFormat(cc_charge,'_')#</b><cfelse><span class="sub">none</span></cfif></td>
			</tr>
...
<cfelseif pgfn EQ "edit">
	...
	<br /><br />
	<span class="pageinstructions">Return to <a href="#CurrentPage#?pgfn=detail&order_ID=#order_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Order #order_number# Detail</a> or <a href="#CurrentPage#?xT=#xT#&xTD=#xTD#&xFD=#xFD#&program_ID=#program_ID#&only_unfulfilled=#only_unfulfilled#&OnPage=#OnPage#">Order List</a> without making changes.</span>
	<br /><br />
	...
	<tr>
	<td colspan="2" class="content2"><b>Award Program:</b> <span class="selecteditem">#FLITC_GetProgramName(FindOrderInfo.this_program_ID)#</span></td>
	</tr>
	...
	<input type="hidden" name="order_note_required" value="You must enter a note explaining why you are editing this order's information."></td>
	</tr>
...
<cfelseif pgfn EQ "additem">
	...
	<cfoutput>
	#ProgramUserInfo(GetUserID.created_user_ID)#
...
<cfelseif pgfn EQ "edititem">
	...
	<cfquery name="FindOrderItem" datasource="#application.DS#">
		SELECT snap_sku, snap_meta_name, snap_productvalue, quantity, snap_options, note AS inv_note
		FROM #application.database#.inventory
		WHERE ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#inventory_ID#" maxlength="10">
			AND is_valid = 1
	</cfquery>
