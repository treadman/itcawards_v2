...
<cfset FLGen_HasAdminAccess(1000000095,true)>
...
<!--- ************************** --->
<cfquery name="GetProgramID" datasource="#application.DS#">
	SELECT program_ID
	FROM #application.database#.admin_users
	WHERE ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#FLGen_adminID#">
</cfquery>
<cfset program_ID=GetProgramID.program_ID>
...
<cfif IsDefined('form.Submit')>
	...
	<cfif form.ID IS NOT "" AND pgfn EQ 'copy'>
		...
		<cfquery name="FindUserAdminAccess" datasource="#application.DS#">
			...
			WHERE lk.user_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.ID#">
	...
	<cfelseif form.ID IS NOT "">
		<cfquery name="UpdateAdminUser" datasource="#application.DS#">
			UPDATE #application.database#.admin_users SET
				firstname = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.firstname#" maxlength="30">,
...
<cfset leftnavon = "sub_admin_user">
...
<cfif pgfn EQ "list">
	...
	<cfquery name="SelectAdminUsers" datasource="#application.DS#">
		...
		AND au.is_system_admin = 0
		<cfif program_ID NEQ ''>
			AND au.program_ID = <cfqueryparam value="#program_ID#" cfsqltype="CF_SQL_INTEGER">
		</cfif>
	...
	<cfoutput query="SelectAdminUsers" group="this_users_program_ID">
		...
		<td>&nbsp;</td>
		<td colspan="3">
		<span class="program_headers">#FLITC_GetProgramName(this_users_program_ID)#</span>
		</td>
		</tr>
		<cfoutput>
			...
			<td><a href="#CurrentPage#?pgfn=edit&id=#ID#">Edit</a>&nbsp;&nbsp;<a href="#CurrentPage#?pgfn=copy&id=#ID#">Copy</a><cfif show_delete>&nbsp;&nbsp;<a href="#CurrentPage#?pgfn=delete&ID=#ID#" onclick="return confirm('Are you sure you want to delete this admin user?  There is NO UNDO.')">Delete</a></cfif></td>
...
<cfelseif pgfn EQ "add" OR pgfn EQ "edit">
	...
	<span class="pageinstructions">Return to <a href="<cfoutput>#CurrentPage#</cfoutput>">Admin User List</a> without making changes.</span>
	<br /><br />
...
<cfelseif pgfn EQ "copy">
	...
	<cfquery name="FindUserAdminAccess" datasource="#application.DS#">
		...
		FROM #application.database#.admin_level al
			LEFT JOIN #application.database#.admin_lookup lk ON al.ID = lk.access_level_ID 
		WHERE lk.user_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ID#">
