	...
	<cfelse>
		...
		<cflock name="productLock" timeout="10">
			...
			<cfquery name="InsertQuery" datasource="#application.DS#">
				INSERT INTO #application.product_database#.product
				...
				<cfquery datasource="#application.DS#" name="getID">
						SELECT Max(ID) As MaxID FROM #application.product_database#.product
				</cfquery>
	...
	<cfloop list="#Form.FieldNames#" index="ThisField">
		<cfif ThisField contains "OPT">
			...
			<cfquery name="InsertSelectedOptions" datasource="#application.DS#">
				INSERT INTO #application.product_database#.product_option
...
<cfif datasaved eq 'yes'>
	<span class="pageinstructions"><a href="#CurrentPage#?pgfn=add&meta_ID=#meta_ID#&xS=#xS#&xL=#xL#&xT=#xT#&xW=#xW#&OnPage=#OnPage#">Add</a> a new Individual Product.</span>
	<br /><br />
</cfif>
...
<cfif pgfn EQ "add">
	...
	<cfquery name="SelectTemplateProd" datasource="#application.DS#">
		SELECT ID AS prod_ID, sku, sortorder, is_dropshipped, is_active, is_discontinued 
...
<cfelseif pgfn EQ "edit" AND IsDefined('prod_ID') AND #prod_ID# IS NOT "">
	...
	<cfquery name="SelectTemplateProd" datasource="#application.DS#">
		SELECT sku, sortorder, is_dropshipped, is_active, is_discontinued 
		FROM #application.product_database#.product
		WHERE ID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#prod_ID#" maxlength="10">
	</cfquery>
	<cfset sku = htmleditformat(SelectTemplateProd.sku)>
...
<form method="post" action="#CurrentPage#">
	...
	<tr class="content">
	<td colspan="2" class="headertext">Product: <span class="selecteditem">#meta_name# <cfif prod_ID NEQ "">#FindProductOptions(prod_ID)##FPO_theseoptions#</cfif></span></td>
	</tr>
	...
	<input type="hidden" name="sku_required" value="You must enter an ITC SKU."></td>
	</tr>
	...
	<cfif FindOptionInfo.RecordCount NEQ 0>
		<tr class="content2"> 
		<td align="right" valign="top">&nbsp;</td>
		<td valign="top"><img src="../pics/contrls-desc.gif" width="7" height="6"> You must select one option from each<br>&nbsp;&nbsp;&nbsp;option category for this individual product.</td>
		</tr>
		<tr class="content">
		...
		<td valign="top">
			...
			<input type="hidden" name="opt#pmoc_ID#_required" value="You must select one option from each option category.">
				<cfoutput>
	...
	<input type="reset" value="Reset" >&nbsp;&nbsp;&nbsp;<input type="submit" name="submit" value="Save <cfif pgfn EQ "edit">Edits<cfelseif pgfn EQ "add">New Individual Product</cfif>" >
