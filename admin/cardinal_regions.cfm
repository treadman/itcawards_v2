...
<cfset FLGen_HasAdminAccess(1000000041,true)>
...
<cfif IsDefined('form.Submit')>
	...
	<cfif form.pgfn EQ "edit">
		<cfquery name="UpdateQuery" datasource="#application.DS#">
			UPDATE #application.database#.cardinal_health_region
	...
	<cfelseif form.pgfn EQ "add">
		<cflock name="cardinal_health_regionLock" timeout="10">
			<cftransaction>
				<cfquery name="InsertQuery" datasource="#application.DS#">
					INSERT INTO #application.database#.cardinal_health_region
				...
				<cfquery name="getID" datasource="#application.DS#">
					SELECT Max(ID) As MaxID FROM #application.database#.cardinal_health_region
				</cfquery>
			</cftransaction>  
		</cflock>
...
<cfset leftnavon = "cardinal_regions">
...
<cfif pgfn EQ "list">
	...
	<span class="pagetitle">Cardinal Health Region List</span>
	<br /><br />
	...
	<cfelse>
		...
		<cfoutput query="SelectList">
			...
			<cfif FLGen_HasAdminAccess(1000000048)>
				<cfquery name="FindLinks" datasource="#application.DS#">
					SELECT COUNT(ID) AS thismany
					FROM #application.database#.cardinal_health_region_lookup
					WHERE region_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#region_ID#" maxlength="10">
				</cfquery>
				<cfif FindLinks.thismany EQ 0><cfset show_delete = true></cfif>
			</cfif>
			<tr class="#Iif(((CurrentRow MOD 2) is 0),de('content2'),de('content'))#">
			<td><a href="#CurrentPage#?pgfn=edit&region_ID=#region_ID#">Edit</a><cfif FLGen_HasAdminAccess(1000000048) AND show_delete>&nbsp;&nbsp;&nbsp;<a href="#CurrentPage#?delete=#region_ID#" onclick="return confirm('Are you sure you want to delete this region?  There is NO UNDO.')">Delete</a></cfif></td>
...
<cfelseif pgfn EQ "add" OR pgfn EQ "edit">
	<!--- START pgfn ADD/EDIT --->
	<cfoutput>
	<span class="pagetitle"><cfif pgfn EQ "add" OR pgfn EQ "copy">Add<cfelse>Edit</cfif> a Cardinal Health Region</span>
	<br /><br />
	<span class="pageinstructions">Return to <a href="#CurrentPage#">Cardinal Health Region List</a> without making changes.</span>
	<br /><br />
	</cfoutput>
	<cfif pgfn EQ "edit">
	...
	<tr class="contenthead">
	<td colspan="2" class="headertext"><cfif pgfn EQ "add">Add<cfelse>Edit</cfif> a Cardinal Health Region</td>
	</tr>
