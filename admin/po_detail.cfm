...
<cfif IsDefined('form.Submit')>
	...
	<cfif IsDefined('form.edit') AND form.edit EQ 'iteminformation'>
		<cfset pgfn = 'editqty'>
		<cfset editinv = inventory_ID>
		<cfif trim(new_quantity) EQ "">
			<cfset alert_msg = 'Please enter a quantity'> 
		<cfelseif not isNumeric(new_quantity)>
			<cfset alert_msg = new_quantity & ' is not a valid quantity.'>
		<cfelse>
			...
			<cflocation addtoken="no" url="po_detail.cfm?pgfn=detail&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=1&datasaved=yes">
		</cfif>
	</cfif>
	<!--- po received --->
	<cfif IsDefined('form.edit') AND form.edit EQ 'porec'>
		<cfset pgfn = 'recpo'>
		<cfif trim(form.recdate) EQ "">
			<cfset alert_msg = 'Please enter a Received date.'>
		<cfelseif NOT isValid("USdate",trim(form.recdate))>
			<cfset alert_msg = form.recdate & ' is not a valid date.'>
		<cfelse>
			...
			<cfif IsDefined('form.FieldNames') AND Trim(form.FieldNames) IS NOT "">
				<cfloop list="#form.FieldNames#" index="FormField">
					<cfif FormField CONTAINS "newqty_">
						<cfset this_qty = Evaluate(FormField)>
						<cfif this_qty NEQ "" AND isNumeric(this_qty)>
							<cfset mod_note = "(*auto* po received)">
							<cfquery name="RecItemQty" datasource="#application.DS#">
								UPDATE #application.database#.inventory
								SET	quantity = <cfqueryparam cfsqltype="cf_sql_integer" value="#this_qty#" maxlength="8">, 
	...
	<cfif IsDefined('form.edit') AND form.edit EQ 'condrop'>
		<cfset pgfn = 'condrop'>
		<cfif trim(form.condrop_date) EQ "">
			<cfset alert_msg = 'Please enter a Confirmation/Shipment date.'>
		<cfelseif NOT isValid("USdate",trim(form.condrop_date))>
			<cfset alert_msg = form.condrop_date & ' is not a valid date.'>
		<cfelse>
						...
						<cfif Evaluate(FormField) NEQ "">
							...
							<cfquery name="ConDropItemQty" datasource="#application.DS#">
								...
								SET tracking = <cfqueryparam cfsqltype="cf_sql_varchar" value="#inventory_tracking_number#" maxlength="32">, 
									po_rec_date = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#trim(form.condrop_date)#">
									#FLGen_UpdateModConcatSQL(mod_note)#
...
<cfif pgfn EQ "detail">
	...
	<br /><br />
	<cfoutput>
	<span class="pageinstructions">Open <a href="po_detail_print.cfm?po_ID=#po_ID#" target="_blank">printable purchase order</a>.</span>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<span class="pageinstructions">Send <a href="po_detail_email.cfm?po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">purchase order via email</a>.</span>
	<br /><br />
	...
	<br /><br />
	<cfif new EQ "yes">
		<cfset alert_msg = "The new Purchase Order #RemoveChars(po_ID,1,5)# was created.">
	</cfif>
	<cfif datasaved EQ "yes">
		<cfset alert_msg = "The change to Purchase Order #RemoveChars(po_ID,1,5)# was saved.#FLGen_SubStamp()#">
	</cfif>
	<cfif GetPOInfo.RecordCount EQ 0 AND was_deleted> 
		<span class="alert">Purchase Order #RemoveChars(po_ID,1,5)# was deleted because there were no more products in it.</span>
	<cfelseif GetPOInfo.RecordCount EQ 0>
		<span class="alert">There is no Purchase Order #RemoveChars(po_ID,1,5)# </span>
	<cfelse>
		<table cellpadding="3" cellspacing="1" border="0" width="100%">
			<tr>
			<td colspan="2" class="contenthead"><b>Purchase Order #RemoveChars(po_ID,1,5)# for #GetPOInfo.snap_vendor#</b> created #GetPOInfo.this_po_date#</td>
			</tr>
			<tr>
			<td colspan="2" class="content2">
				<a href="#CurrentPage#?pgfn=editpo&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">Edit PO Information</a>&nbsp;&nbsp;&nbsp;
				<cfif GetPOInfo.is_dropship EQ 1 AND GetPOInfo.po_rec_date EQ "">
					<a href="#CurrentPage#?pgfn=condrop&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">PO - Confirm Dropship</a>
				...
				<cfelseif GetPOInfo.is_dropship EQ 0 AND GetPOInfo.po_rec_date EQ "">
					<a href="#CurrentPage#?pgfn=recpo&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">PO - Inventory Received</a>
				<cfelse>
			...
			<cfloop query="GetPOInvItems">
				...
				<td>
					<cfif GetPOInfo.is_dropship EQ 1 AND  GetPOInvItems.po_rec_date EQ "">
						<a href="#CurrentPage#?xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#&rex=#GetPOInvItems.inventory_ID#&po_ID=#po_ID#">remove</a>
					...
					<cfelseif GetPOInfo.is_dropship EQ 0 AND GetPOInvItems.po_rec_date EQ "">
						<a href="#CurrentPage#?pgfn=editqty&editinv=#inventory_ID#&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">Edit&nbsp;Qty</a>
					<cfelse>
				...
				<cfif GetPOInfo.is_dropship EQ 1>
					...
					<td>
						#FLITC_GetProgramName(OrderInfo.program_ID)# Order #OrderInfo.order_number#<br>
						<cfif hasForwarding>
							Ship to forwarder address:	#UserForwarding.company#, #UserForwarding.address1# #UserForwarding.address2#, #UserForwarding.city#, #UserForwarding.state# #UserForwarding.zip#
							<br>
							Forward to:
						<cfelse>
							SHIP TO:
						</cfif>
						#OrderInfo.snap_ship_fname# #OrderInfo.snap_ship_lname#, #OrderInfo.snap_ship_address1# #OrderInfo.snap_ship_address2#, #OrderInfo.snap_ship_city#, #OrderInfo.snap_ship_state# #OrderInfo.snap_ship_zip#, phone: #OrderInfo.snap_phone#, email: #OrderInfo.snap_email#
					</td>
					</tr>
				<cfelseif GetPOInfo.is_dropship EQ 0>
					...
					<cfquery name="FindOrders" datasource="#application.DS#">
						...
						JOIN #application.database#.order_info o ON i.order_ID = o.ID 
					...
					<cfif FindOrders.RecordCount GT 0>
						...
						<td colspan="3" class="selectedbgcolor">
							...
							<span ID="row_#ID#" style="display:none"><cfloop query="FindOrders"><a href="order.cfm?pgfn=detail&order_ID=#order_ID#&xT=&xTD=&xFD=&program_ID=&OnPage=1">#FLITC_GetProgramName(program_ID)# Order #order_number#</a> #FLGen_DateTimeToDisplay(created_datetime)#<br />#snap_fname# #snap_lname#&nbsp;&nbsp;&nbsp;#snap_email#&nbsp;&nbsp;&nbsp;#snap_phone#<br /><br /></cfloop></span>
						</td>
						</tr><!--- order_ship.cfm?back=order&shipID=1000000309&OnPage=1 --->
					<cfelse>
						<tr class="content">
						<td>&nbsp;</td>
						<td>&nbsp;</td>
...
<cfelseif pgfn EQ "editpo">
	...
	<br /><br />
	<span class="pageinstructions">Return to <a href="#CurrentPage#?pgfn=detail&po_ID=#po_ID#&xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">Purchase Order #RemoveChars(po_ID,1,5)# Detail</a> or <a href="po_list.cfm?xT=#xT#&xTD=#xTD#&xFD=#xFD#&v_ID=#v_ID#&OnPage=#OnPage#">Purchase Order List</a> without making changes.</span>
	<br /><br />
	...
	<td valign="top">#RemoveChars(po_ID,1,5)#</td>
	</tr>
	<tr class="content">
...
<cfelseif pgfn EQ "recpo">
	...
	<table cellpadding="3" cellspacing="1" border="0" width="100%">
		<tr>
		<td colspan="2" class="contenthead"><b>Purchase Order #RemoveChars(po_ID,1,5)# for #GetPOInfo.snap_vendor#</b></td>
		</tr>
		<tr class="content">
		<td>
			For: #HTMLEditFormat(GetPOInfo.snap_vendor)#<br>
	...
	<cfloop query="GetPOInvItems">
		...
		<tr class="content">
		<td align="center">
			<cfif snap_is_dropshipped EQ 0 AND rec_qty EQ 0>
				<cfset this_val = "">
				<cfif isDefined("form.newqty_#inventory_ID#")>
					<cfset this_val = evaluate("form.newqty_#inventory_ID#")>
				</cfif>
				<input type="text" name="newqty_#inventory_ID#" value="#this_val#" maxlength="8" size="5">
			<cfelseif snap_is_dropshipped EQ 0 AND rec_qty NEQ 0>
				recieved #rec_qty#<br>#FLGen_DateTimeToDisplay(po_rec_date)#
			<cfelse>
	...
	<cfloop query="GetPOInvItems">
		...
		<td align="center">
			<cfif snap_is_dropshipped EQ 1 AND tracking EQ "">
				...
				<input type="text" name="inv_#inventory_ID#" value="#this_val#" maxlength="32" size="14">
		...
		<td>
			...
			#OrderInfo.snap_ship_fname# #OrderInfo.snap_ship_lname#, #OrderInfo.snap_ship_address1# #OrderInfo.snap_ship_address2#, #OrderInfo.snap_ship_city#, #OrderInfo.snap_ship_state# #OrderInfo.snap_ship_zip#, phone: #OrderInfo.snap_phone#
		</td>
		</tr>
	</cfloop>
	<tr class="content">
