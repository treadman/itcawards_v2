...
<cfset FLGen_HasAdminAccess("1000000074-1000000080",true)>
...
<cfif IsDefined('form.Submit')>
	...
	<cfif pgfn EQ "add" OR pgfn EQ "copy">
		<cflock name="email_templatesLock" timeout="10">
			<cftransaction>
				<cfquery name="InsertQuery" datasource="#application.DS#">
					INSERT INTO #application.database#.email_templates
				...
				<cfquery name="getID" datasource="#application.DS#">
					SELECT Max(ID) As MaxID FROM #application.database#.email_templates
				</cfquery>
			</cftransaction>  
		</cflock>
	...
	<cfelseif pgfn EQ "edit">
		...
		<cfquery name="UpdateQuery" datasource="#application.DS#">
			UPDATE #application.database#.email_templates
		...
		<cfquery name="DeleteAssignedXrefs" datasource="#application.DS#">
			DELETE FROM #application.database#.xref_program_email
			WHERE email_alert_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.ID#" maxlength="10">
		</cfquery>
	</cfif>
	...
	<cfif IsDefined('form.assign_xref') AND form.assign_xref IS NOT "">
		<cfloop list="#form.assign_xref#" index="i">
			<cfquery name="InsertTheseXref" datasource="#application.DS#">
				INSERT INTO #application.database#.xref_program_email
				(created_user_ID, created_datetime, email_alert_ID, program_ID)
				VALUES (
				<cfqueryparam cfsqltype="cf_sql_integer" value="#FLGen_adminID#" maxlength="10">,
				#FLGen_DateTimeToMySQL()#, 
	...
	<cfset datasaved = "yes">
	<cfset pgfn = "edit">
<cfelseif delete NEQ '' AND FLGen_HasAdminAccess(1000000074)>
...
--->
	...
	<cfquery name="SelectList" datasource="#application.DS#">
		SELECT ea.ID, Date_Format(ea.created_datetime,'%c/%d/%Y') AS created_date, 
			<!--- COUNT(xau.ID) AS number_of_emails_sent, --->
<!---			SELECT COUNT(xau.ID) FROM #application.database#.xref_alerts_users xau WHERE xau.alert_ID = ea.ID) AS number_of_emails_sent,--->
			p.company_name, p.program_name, et.email_title
		FROM #application.database#.email_alerts ea
		LEFT JOIN #application.database#.admin_users au ON ea.created_user_ID = au.ID 
		LEFT JOIN #application.database#.program p ON p.ID = ea.program_ID 
		LEFT JOIN #application.database#.email_templates et on et.id = ea.template_ID
		<!--- LEFT JOIN #application.database#.xref_alerts_users xau on xau.alert_ID = ea.ID --->
<!---
		WHERE 1=1 
		<cfif program_ID NEQ "">
			AND ea.program_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#program_ID#">
		</cfif>
...
<!---	<cfdump var="#SelectList#"><cfabort>--->
	...
	<span class="pagetitle">Email Alert Report</span>
<!---
	<br /><br />
	<!--- search box --->
	...
	<td class="content" colspan="2" align="center">
		...
		<table cellpadding="5" cellspacing="0" border="0" width="100%">
		<tr>
	<!---<cfif NOT FLGen_HasAdminAccess(1000000075)><td align="center"><span class="sub">Award Program</span><br>#SelectProgram(program_ID,"All Award Programs")#</td></cfif>--->
		<td align="right">
<!---
...
--->
				...
				<td valign="top" width="50%">#company_name# <span class="sub">[#program_name#]</span></td>
				<td valign="top" width="50%">#email_title#</td>
				</tr>
			</cfoutput>
		</cfif>
	</table>
...
<cfelseif pgfn EQ "details">
	...
	<br /><br />
	<span class="pageinstructions">Return to <a href="#CurrentPage#?program_ID=#program_ID#&xFD=#xFD#&xTD=#xTD#&OnPage=#OnPage#">Email Alert Report</a>.</span>
	<br /><br />
	</cfoutput>
	...
	<td valign="top">#template_text#</span></td>
	</tr>
	...
	<cfquery name="FindRecipients" datasource="#application.DS#">
		...
		LEFT JOIN #application.database#.program_user up ON xau.user_ID = up.ID
		WHERE xau.alert_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ID#">
		<cfif FLGen_HasAdminAccess(1000000075)>
			<cfset ThisID = ListGetAt(itc_program,1,'-')>
			AND (SELECT COUNT(ID) FROM #application.database#.email_alerts ea WHERE ea.program_ID = #ThisID# and ea.ID = xau.alert_ID) = 1 
		</cfif>
	</cfquery>
