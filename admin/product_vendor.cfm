...
<cfparam name="dropship_fee" default="">
<cfparam name="freight" default="">

<cfset vendor_cost = replace(vendor_cost,"$","","ALL")>
<cfset vendor_cost = replace(vendor_cost,",","","ALL")>
<cfset dropship_fee = replace(dropship_fee,"$","","ALL")>
<cfset dropship_fee = replace(dropship_fee,",","","ALL")>
<cfset freight = replace(freight,"$","","ALL")>
<cfset freight = replace(freight,",","","ALL")>
<cfif not isNumeric(dropship_fee)>
	<cfset dropship_fee = 0>
</cfif>
<cfif not isNumeric(freight)>
	<cfset freight = 0>
</cfif>
	...
	<cfif IsDefined('form.vl_ID') AND form.vl_ID IS NOT "">
		...
		<cfif NOT is_dropshipped>
			...
			<cfif (Trim(pack_size) IS "" AND NOT Trim(pack_desc) IS "") OR (Trim(pack_desc) IS "" AND NOT Trim(pack_size) IS "")>
				<cfset pack_size = "">
				<cfset pack_desc = "">
				<cfset pack_error = "You must enter both a Pack Size and a Pack Description.  Please try again.">
			</cfif>
		</cfif>
		<!--- do vendor lookup update --->
		<cfquery name="UpdateVendorlookup" datasource="#application.DS#">
			UPDATE #application.product_database#.vendor_lookup
			SET	vendor_ID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#vendor_ID#" maxlength="10">,
				...
				vendor_PO_note = <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#vendor_PO_note#" null="#YesNoFormat(NOT Len(Trim(vendor_PO_note)))#">,
				...
				dropship_fee = <cfqueryparam cfsqltype="cf_sql_decimal" scale="2" value="#dropship_fee#" maxlength="12" null="#YesNoFormat(NOT Len(Trim(dropship_fee)))#">,
				freight = <cfqueryparam cfsqltype="cf_sql_decimal" scale="2" value="#freight#" maxlength="12" null="#YesNoFormat(NOT Len(Trim(freight)))#">,
		...
		<cflock name="vendor_lookupLock" timeout="10">
			<cftransaction>
				<cfquery name="InsertVendorlookup" datasource="#application.DS#">
					INSERT INTO #application.product_database#.vendor_lookup
						(created_user_ID, created_datetime, vendor_ID, product_ID, is_default, vendor_sku, vendor_PO_note, vendor_cost, dropship_fee, freight, vendor_min_qty, pack_size, pack_desc)
					VALUES (
						...
						<cfqueryparam cfsqltype="cf_sql_longvarchar" value="#vendor_PO_note#" null="#YesNoFormat(NOT Len(Trim(vendor_PO_note)))#">, 
						<cfqueryparam cfsqltype="cf_sql_decimal" scale="2" value="#vendor_cost#" maxlength="12" null="#YesNoFormat(NOT Len(Trim(vendor_cost)))#">, 
						<cfqueryparam cfsqltype="cf_sql_decimal" scale="2" value="#dropship_fee#" maxlength="12" null="#YesNoFormat(NOT Len(Trim(dropship_fee)))#">, 
						<cfqueryparam cfsqltype="cf_sql_decimal" scale="2" value="#freight#" maxlength="12" null="#YesNoFormat(NOT Len(Trim(freight)))#">, 
				...
				<cfquery datasource="#application.DS#" name="getID">
					SELECT Max(ID) As MaxID FROM #application.product_database#.vendor_lookup
				</cfquery>
				<cfset vl_ID = getID.MaxID>
			</cftransaction>  
		</cflock>
	</cfif>
	<cflocation addtoken="no" url="product.cfm?pgfn=edit&meta_ID=#meta_ID#&xS=#xS#&xL=#xL#&xT=#xT#&xW=#xW#&xA=#xA#&OnPage=#OnPage#&alert_msg=The%20vendor%20info%20was%20saved.">
...
<cfelseif IsDefined('form.Submit2')>
	...
	<cflocation addtoken="no" url="product.cfm?pgfn=edit&meta_ID=#meta_ID#&xS=#xS#&xL=#xL#&xT=#xT#&xW=#xW#&xA=#xA#&OnPage=#OnPage#&alert_msg=The%20vendor%20info%20was%20saved%20for%20all%20individual%20products.">
</cfif>
...
<br /><br />
<span class="pageinstructions">Return to <a href="<cfoutput>product.cfm?pgfn=edit&meta_ID=#meta_ID#&xS=#xS#&xL=#xL#&xT=#xT#&xW=#xW#&xA=#xA#&OnPage=#OnPage#</cfoutput>">Product Detail</a> or <a href="<cfoutput>product.cfm?&xS=#xS#&xL=#xL#&xT=#xT#&xW=#xW#&xA=#xA#&OnPage=#OnPage#</cfoutput>">Product List</a> without making changes.</span>
<br /><br />
...
<cfif pack_error NEQ "">
	<span class="alert"><cfoutput>#pack_error#</cfoutput></span>#FLGen_SubStamp()#
	<br /><br />
</cfif>
...
<cfif IsDefined('vl_ID') AND vl_ID IS NOT "">
	...
	<cfquery name="ToBeEdited" datasource="#application.DS#">
		SELECT vendor_ID, is_default, vendor_sku, vendor_PO_note, vendor_cost, dropship_fee, freight, vendor_min_qty, pack_size, pack_desc
	...
	<cfset dropship_fee = htmleditformat(ToBeEdited.dropship_fee)>
	<cfset freight = htmleditformat(ToBeEdited.freight)>
...
<form method="post" action="#CurrentPage#" name="TheOnlyForm">
	...
	<tr class="content">
	<td colspan="2" class="headertext">Product: <span class="selecteditem">[SKU: #prod_sku#]</span> <span class="selecteditem">#meta_name# #FindProductOptions(prod_ID)##FPO_theseoptions#</span></td>
	</tr>
	...
	<td valign="top">
		...
		<input type="hidden" name="vendor_ID_required" value="You must choose a vendor from the dropdown.">
	</td>
	</tr>
	...
	<td valign="top"><textarea name="vendor_PO_note" rows="5" cols="60">#vendor_PO_note#</textarea></td>
	</tr>
	...
	<input type="hidden" name="vendor_cost_required" value="You must enter an ITC cost."></td>
	</tr>

	<tr class="content">
	<td align="right">Dropship Fee: </td>
	<td valign="top"><input type="text" name="dropship_fee" maxlength="10" size="12" value="<cfif dropship_fee EQ ''>0<cfelse>#dropship_fee#</cfif>"> <span class="sub">(only enter numbers and decimal, ex. 30 or 30.25)</span>
	</tr>

	<tr class="content">
	<td align="right">Freight: </td>
	<td valign="top"><input type="text" name="freight" maxlength="10" size="12" value="<cfif freight EQ ''>0<cfelse>#freight#</cfif>"> <span class="sub">(only enter numbers and decimal, ex. 30 or 30.25)</span>
	</tr>
	...
	<cfif NOT SelectProdInfo.is_dropshipped>	
		...
		<td valign="top"><input type="text" name="pack_size" maxlength="6" size="8" value="#pack_size#">
			<span class="sub">(If you have a pack size, you must have a description.)</span></td>
		</tr>
		<tr class="content">
		<td align="right">Vendor&nbsp;Pack&nbsp;Description: </td>
		<td valign="top"><input type="text" name="pack_desc" maxlength="16" size="8" value="#pack_desc#"> <span class="sub">(If you have a pack size, you must have a description.)</span></td>
		</tr>
	<cfelse>
